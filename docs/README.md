...
- **The `CardSet` / `CardRarity` enums are partially ahead of the data.** `Vendetta` released 2026-07-31 but its CSV hasn't been imported into `cards.json` yet. `Radiance` (expected 2026-10-23) and `Ultimate` rarity are in the enum/validSets but absent from the dataset. Check [game-background.md](./game-background.md) for the current release schedule before treating an enum member as "in the data."
